...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // Support both Lovable gateway and direct Gemini API if configured
    ...
    // Prefer Gemini Direct if key is present
    if (GEMINI_API_KEY) {
       response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${GEMINI_API_KEY}`, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `${SYSTEM_PROMPT}\n\nAnalyze this diagnostic data and provide root cause analysis:\n\n${JSON.stringify(diagnosticSummary, null, 2)}`
            }]
          }]
        })
      });
