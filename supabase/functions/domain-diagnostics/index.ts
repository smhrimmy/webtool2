...
// Fast DNS query with aggressive timeout
...
// Fast WHOIS fetch with aggressive timeouts
async function fetchWHOIS(domain: string) {
  ...
  try {
    ...
    if (rdapServer) {
      console.log(`Fetching RDAP from ${rdapServer} for ${domain}`);
      try {
        ...
        console.log(`Direct RDAP failed for ${domain}, trying bootstrap`);
      }
    }
    
    // Fallback: Try RDAP bootstrap service
    console.log(`Trying RDAP bootstrap for ${domain}`);
    try {
      ...
      console.log(`RDAP bootstrap also failed for ${domain}`);
    }
...
// Fast website check with shorter timeout
async function checkWebsite(domain: string) {
  try {
    ...
    // Start IP lookup in parallel with website fetch
...
// Fetch real SSL certificate data from crt.sh Certificate Transparency logs
async function fetchCertificateFromCTLogs(domain: string) {
  try {
    console.log(`Fetching certificate from crt.sh for ${domain}`);
    
    // Use a single focused query - direct domain search works best
    const controller = new AbortController();
    ...
    try {
      ...
      if (!crtResponse.ok) {
        console.log(`crt.sh returned status ${crtResponse.status}`);
        return null;
      }
      ...
      if (!Array.isArray(certificates) || certificates.length === 0) {
        console.log(`No certificates found for ${domain}`);
        return null;
      }
      ...
      // Find the most recent valid certificate that matches the domain
      ...
      const validCerts = uniqueCerts.filter((cert: any) => {
        ...
        // Check if the certificate is for this domain
      ...
      if (validCerts.length > 0) {
        console.log(`Found ${validCerts.length} valid certs for ${domain}, using most recent`);
        return validCerts[0];
      }
      
      // Fallback to most recent cert even if expired
      ...
      console.log(`No valid certs, using most recent for ${domain}`);
      return uniqueCerts[0] || null;
      
    } catch (e: any) {
      clearTimeout(timeout);
      console.log(`crt.sh fetch failed for ${domain}:`, e.message || e);
      return null;
    }
  } catch (err: any) {
    console.error(`crt.sh lookup failed for ${domain}:`, err.message);
    return null;
  }
}

// Parse certificate issuer from crt.sh format
function parseIssuer(issuerName: string): string {
  if (!issuerName) return 'Unknown';
  
  // Extract organization name from issuer string
  // Format: "C=XX, O=Organization Name, CN=Common Name"
...
function parseSANDomains(nameValue: string): string[] {
  ...
  return domains.slice(0, 20); // Limit to 20 SANs
}

async function checkSSL(domain: string) {
  try {
    // Fetch real certificate data from CT logs
    const ctCert = await fetchCertificateFromCTLogs(domain);
    
    // Also check HTTPS connectivity for HSTS
    const controller = new AbortController();
    ...
    try {
      ...
      clearTimeout(timeout);
      console.log(`HTTPS check failed for ${domain}, using CT data only`);
    }
    
    // Parse HSTS header for details
    let hstsMaxAge = 0;
    ...
    // Use CT log data for certificate details
    if (ctCert) {
      ...
      console.log(`Found certificate for ${domain}: issuer=${issuer}, expires=${expiryDate}, SANs=${sanDomains.length}`);
    ...
    // Fallback if no CT data available
    return {
      valid: httpsValid,
    ...
    console.error(`SSL check failed for ${domain}:`, err.message);
    return {
      valid: false,
  ...
  try {
    ...
    if (restApiOk && restApi.status === 'fulfilled') {
      try {
        ...
        // WP version is sometimes in generator or namespaces
        if (restData?.namespaces?.includes('wp/v2')) {
          version = 'WP 4.7+'; // REST API v2 was introduced in WP 4.7
        }
      } catch {}
    }
    ...
    console.error(`WordPress check failed:`, err.message);
    return { 
      detected: false, 
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Running diagnostics for: ${cleanDomain}`);

    // Launch ALL operations simultaneously for maximum speed
    // DNS queries - use only fastest resolvers (first 2 per region) for speed
    ...
    // Start ALL operations in parallel - don't wait for phases
    ...
    console.log(`Diagnostics complete for ${cleanDomain}`);
